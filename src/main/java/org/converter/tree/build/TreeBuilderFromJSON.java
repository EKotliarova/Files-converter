...
@Slf4j
@Component
public class TreeBuilderFromJSON implements TreeBuilderFromFile {
    ...
    public Tree getOrUpdateTreeFromFile(String filePath, Optional<Tree> tree) {
        ...
        try (BufferedReader reader = new BufferedReader(new java.io.FileReader(filePath))) {
            ...
            return tree.isPresent()
                    ? new Tree(mapper.readerForUpdating(tree.get().getRoot()).readTree(reader))
                    : new Tree(mapper.readTree(reader));
        } catch (IOException e) {
